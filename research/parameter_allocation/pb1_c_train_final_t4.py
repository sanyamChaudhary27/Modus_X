...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(
        description="PB1-C FINAL T4 trainer — low-rank Q/K with 32-token truncated BPTT"
    )
...
def main() -> None:
    ...
    print("=" * 80)
    print("MODUS_X PB1-C — FINAL T4 LOW-RANK Q/K TRAINING")
    print("=" * 80)
    ...
    print("T4 execution mode            : batch-1 on one visible T4")
    ...
    # PB1-C state is intentionally batch-1: one continuous recurrent state
    # is carried across optimizer updates. On Kaggle T4x2 we therefore pin
    # this experiment to one T4 rather than silently changing the optimizer
    # protocol to data-parallel batch-2 training.
    if args.batch != 1:
        raise ValueError(
            "PB1-C T4 requires --batch 1 to preserve the declared "
            "continuous-state / 32-token truncated-BPTT protocol."
        )
    ...
    # Keep the stateful PB1-C update on one device. Using both GPUs here
    # would require changing batch/state semantics and therefore changing
    # the experiment. The second T4 remains available for a future
    # explicitly controlled data-parallel experiment.
    train_device = all_devices[0]

    print(f"Accelerator backend            : {jax.default_backend()}")
    print(f"Visible accelerator devices    : {len(all_devices)}")
    for i, device in enumerate(all_devices):
        print(f"  device[{i}]                  : {device}")
    print(f"PB1-C training device         : {train_device}")
    print("T4x2 mode                     : single-device batch-1")
    print("Reason                        : preserves continuous recurrent-state protocol")

    mesh = Mesh(
        np.array([train_device], dtype=object),
        ("data",),
    )
