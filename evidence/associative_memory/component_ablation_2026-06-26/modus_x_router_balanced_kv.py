...
def model_fwd(p: dict, seq: jax.Array, cfg: Config, fusion: str = "convex") -> jax.Array:
    ...
    if router.shape[0] == 1:
        router = router[0]
    if fusion == "matrix_only":
        fused = memory_read
    elif fusion == "vector_only":
        fused = vector_read
    elif fusion == "convex":
...
def make_model(name: str, key: jax.Array, cfg: Config):
    if name == "VectorPM":
        ...
        fusion = "convex"
    elif name == "MatrixOnly":
        # Match the lean-vector allocation exactly, then intervene only on the
        # fused output so differences isolate stream usage rather than width.
        params, _ = make_model("VectorLeanPM", key, cfg)
        fusion = "matrix_only"
    elif name == "VectorOnly":
        params, _ = make_model("VectorLeanPM", key, cfg)
        fusion = "vector_only"
    else:
