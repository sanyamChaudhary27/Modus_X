"""Frozen seed-1 2000-update execution replication; validation only."""
import argparse
import hashlib
import json
import pickle
...
def main():
    ...
    for path in ([a.checkpoint] if a.checkpoint else Path('/kaggle/input').rglob('checkpoint.pkl')):
        ...
        if args.get('seed')!=1: continue
        choices.setdefault(digest(path),(path,doc,args))
    if len(choices)!=1:
        raise RuntimeError('Attach ONE original SEED-1 trained 47M v3 SegmentRetention endpoint with checkpoint.pkl and config.json. Seed2 and synthetic checkpoints cannot be used for independent replication. Candidates: '+str([str(v[0]) for v in choices.values()]))
    ...
    assert saved['step']==25000 and 'rng_state' in saved, 'Need original step25000 seed1 endpoint'
    ...
    report={'source_checkpoint':str(path),'source_sha256':sha,'source_config':config,
        'start_step':start_step,'seed':1,'updates':2000,'added_characters':8192000,'test_evaluated':False,
    ...
    report['devices']=list(map(str,jax.devices()))
    report['endpoint_artifacts']={}
    report['checkpoint_restore']={}
    def save_checkpoint(name,state,step):
        checkpoint=a.outdir/f'{name}_step{start_step+step}_checkpoint.pkl'
        partial=checkpoint.with_suffix('.partial')
        p,s=state
        with partial.open('wb') as f:
            pickle.dump({'params':jax.device_get(p),'opt_state':jax.device_get(s),
                'step':start_step+step,'rng_state':rng.bit_generator.state,
                'source_config':config,'source_sha256':sha,'execution_backend':name},f,
    ...
    try:
        ...
        for step in range(1,2001):
            ...
            if step==1000:
                # Clone the cursor: diagnostic next-batch generation must not consume training RNG.
                replay_rng=np.random.default_rng(); replay_rng.bit_generator.state=rng.bit_generator.state
                replay_starts=replay_rng.integers(0,len(train)-513,size=8)
                rx,ry=batch_at(train,replay_starts,512)
                rx=jax.device_put(rx,bs); ry=jax.device_put(ry,bs)
                for name,state in list(states.items()):
                    checkpoint=save_checkpoint(name,state,step)
                    with checkpoint.open('rb') as f: restored=pickle.load(f)
                    assert restored['step']==start_step+step
                    assert restored['rng_state']==rng.bit_generator.state
                    loaded=jax.device_put((restored['params'],restored['opt_state']),rep)
                    del restored
                    expected=compiled[name](*state,rx,ry)
                    actual=compiled[name](*loaded,rx,ry)
                    check=errors(expected,actual)
                    report['checkpoint_restore'][name]=check
                    assert check['max_abs']==0.,check
                    states[name]=loaded
                    del expected,actual,loaded
                    print('REPLICATION_RESTORE_PASS',name,json.dumps(check),flush=True)
            if step%500==0:
                (a.outdir/'replication_result.json').write_text(json.dumps(report,indent=2))
        # Preserve full endpoints before long validation; compact ZIP still excludes pickles.
        for name,state in states.items():
            checkpoint=save_checkpoint(name,state,2000)
            report['endpoint_artifacts'][name]={'file':checkpoint.name,'bytes':checkpoint.stat().st_size,'sha256':digest(checkpoint)}
        (a.outdir/'checkpoint_manifest.json').write_text(json.dumps(report['endpoint_artifacts'],indent=2))
        ...
        report['replication_pass']=report['screen_pass'] and all(v['max_abs']==0. for v in report['checkpoint_restore'].values()) and len(report['checkpoint_restore'])==2
        print('TWO_STAGE_REPLICATION_DECISION',json.dumps({k:v for k,v in report.items() if k not in ('rows','source_config')}),flush=True)
    finally:
