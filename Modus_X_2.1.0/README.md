...
Status: **v2.1.0 release candidate**

Modus_X 2.1.0 studies whether bounded associative matrix memory can complement
...
- The measured MemoryFeedback scaling curve improves from `47.44M` to
  ...
  `102.4M`-character budget. Matched late annealing substantially improves
  both larger checkpoints.
- At the matched `163.84M`-character endpoint, the promoted `81.49M`
  MemoryFeedback model reaches `1.382445` dense test BPC. It narrowly improves
  v1.1.1 (`1.384180`) with `1.54%` fewer parameters, remains behind official
  Mamba, and remains ahead of official xLSTM.
...
The published lineage begins with Modus_X v1.1.1:
`https://doi.org/10.5281/zenodo.20923248`. Modus_X v2.0.0 is preserved at
`https://doi.org/10.5281/zenodo.21538210`. The v2.1.0 version DOI must be
inserted only after Zenodo reserves the new-version draft.
