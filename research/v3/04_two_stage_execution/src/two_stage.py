...
def forward(layer, x, initial_state=None, associative=False, archive_clock=1.0):
    ...
    current, archive, vector = initial_state
    ...
    key = jax.vmap(normalize)(project('m_wk'))
    query = jax.vmap(normalize)(project('m_wq'))
    value = jnp.tanh(project('m_wv'))
    ...
    return (current, archive, vector), router * matrix_out + (1 - router) * vector_out
