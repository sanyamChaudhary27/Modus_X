...
def main():
    ...
    ap.add_argument('--seed',type=int,choices=(3,),required=True)
    ap.add_argument('--resume',action='store_true')
    a=ap.parse_args(); a.outdir.mkdir(parents=True,exist_ok=True)
    if a.seed != 3 or a.resume:
        raise ValueError('This frozen replication starts seed3 from scratch only.')
    ...
    protocol_path=Path(__file__).with_name('SEED3_PROTOCOL.json')
    report['frozen_protocol']=json.loads(protocol_path.read_text())
    report['protocol_sha256']=hashlib.sha256(protocol_path.read_bytes()).hexdigest()
    print('LOCAL_KV_FROZEN_PROTOCOL',json.dumps(report['frozen_protocol']),flush=True)
    ...
    try:
        ...
        for step in range(start+1,25001):
            ...
            if step==5000:
                ...
                print('LOCAL_KV_FIXED_HORIZON_CONTINUE; screen recorded, fixed25k endpoint unchanged',flush=True)
        dense={}
