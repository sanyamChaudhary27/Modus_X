...
def main() -> None:
    ...
    parser.add_argument("--matched-endpoint", type=Path, required=True)
    ...
    endpoint = json.loads(args.matched_endpoint.read_text(encoding="utf-8"))
    expected_endpoint = {
        "params": 81_486_728,
        "processed_characters": 163_840_000,
    ...
    mismatches = {
        key: {"expected": value, "found": endpoint.get(key)}
        for key, value in expected_endpoint.items()
        if endpoint.get(key) != value
    }
    if mismatches:
        raise ValueError(f"Matched endpoint metadata mismatch: {mismatches}")
    ...
    matched_rows = [
        ...
        {
            "label": "MemoryFeedback v2",
            "params": endpoint["params"],
            "dense_test_bpc": endpoint["dense_test_bpc"],
            "color": "#7a5195",
        },
        {
            "label": "Modus X v1.1.1",
    ...
    matched_rows.sort(key=lambda row: row["dense_test_bpc"])
    matched_figure, matched_axis = plt.subplots(figsize=(10.8, 5.8))
    positions = list(range(len(matched_rows)))
    for position, row in zip(positions, matched_rows):
        matched_axis.scatter(
            row["dense_test_bpc"],
            position,
            s=145,
            color=row["color"],
        ...
        matched_axis.text(
            row["dense_test_bpc"] + 0.0012,
            position,
            f"{row['dense_test_bpc']:.6f}",
            va="center",
            fontsize=10,
        )
    matched_axis.set_yticks(
        positions,
        [
            f"{row['label']}  |  {row['params'] / 1e6:.2f}M params"
    ...
    matched_axis.invert_yaxis()
    matched_axis.set_xlabel("Dense test bits per character (lower is better)")
    matched_axis.set_title(
        "Matched enwik8 endpoint: 163.84M characters, 40,000 updates"
    )
    matched_axis.grid(axis="y", visible=False)
    matched_axis.set_xlim(1.338, 1.431)
    matched_figure.tight_layout()
    matched_figure.savefig(
        args.outdir / "memory_feedback_matched_endpoint.png",
        dpi=180,
    )
    plt.close(matched_figure)
