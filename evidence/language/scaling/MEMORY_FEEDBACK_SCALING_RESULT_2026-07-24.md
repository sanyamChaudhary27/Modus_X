...
Status: complete through the matched `163.84M` endpoint. MemoryFeedback 81M
is promoted as the v2 language endpoint.
...
## Annealing diagnosis at 122.88M characters

Both saved checkpoints were continued from step `25,000` to step `30,000` at
LR `1e-4` and dense-audited with the same dataset and protocol.

| Model | Parameters | Dense train-tail | Dense validation | Dense test | Training time |
|---|---:|---:|---:|---:|---:|
| MemoryFeedback 81M | `81,486,728` | `1.304409` | **`1.398211`** | `1.408172` | `31,405s` |
| MemoryFeedback 99M | `99,438,920` | **`1.297654`** | `1.398734` | **`1.404835`** | `32,671s` |

Relative to the `102.4M` endpoint:

- 81M improves dense validation by `0.034927` and dense test by `0.035700`;
- 99M improves dense validation by `0.035549` and dense test by `0.037199`.

The continuation confirms that both models were under-annealed at `102.4M`.
It does not rescue the 99M scaling claim. At `122.88M`, 99M is `0.000523`
worse on dense validation and only `0.003337` better on dense test while using
`22.03%` more parameters and about `4.03%` more training time.

The pre-registered 99M promotion gate required at least a `0.005` validation
advantage with no test loss. It fails. Freeze the 99M checkpoint as a measured
near-tie and continue the more efficient 81M branch.
...
Continue the winning 81M efficiency branch to the existing `163.84M` matched
endpoint:
...
The 99M checkpoint failed the annealing promotion gate and is frozen. After
the matched 81M endpoint, test at most one fresh scale-corrected 99M candidate:
proportional matrix/vector state width and feedback rank 48, with parameter
and runtime matching pre-registered before training. Do not tune that
candidate from final test data.
...
## Matched 163.84M endpoint

The promoted 81M checkpoint was continued from step `30,000` to `35,000` at
LR `7e-5`, then to step `40,000` at LR `5e-5`. The final checkpoint was
dense-audited with the same enwik8 splits, 512-token context, and offsets 0
and 256.

| Model | Parameters | Characters | Dense validation | Dense test |
|---|---:|---:|---:|---:|
| Official Mamba | `81,462,656` | `163.84M` | **`1.350538`** | **`1.345780`** |
| MemoryFeedbackArchive v2 | `81,486,728` | `163.84M` | `1.375422` | `1.382445` |
| Modus_X v1.1.1 | `82,764,964` | `163.84M` | `1.378681` | `1.384180` |
| Official xLSTM | `76,649,664` | `163.84M` | `1.435132` | `1.419620` |

Relative to v1.1.1, MemoryFeedbackArchive improves dense validation by
`0.003259` BPC and dense test by `0.001735` BPC while using `1.54%` fewer
parameters. Official Mamba remains better by `0.036665` dense test BPC;
MemoryFeedbackArchive is better than official xLSTM by `0.037175`.

This is a narrow but matched v2 language win over the published Modus_X line,
not a universal architecture or SOTA claim. It is one seed and the margin
over v1.1.1 is small. The result should be replicated before attributing the
entire difference to MemoryFeedback rather than run variance.

Decision: promote the 81M checkpoint as the v2 language endpoint, freeze the
99M branch, and stop enwik8 continuation for release. The next language study
must be either a matched replication or a new-data/downstream evaluation, not
further test-guided annealing.
